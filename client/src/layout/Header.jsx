                ...
                <div className="navbar-start">
                    ...
                    <div className="dropdown lg:hidden">
                        ...
                        <div tabIndex={0} className="menu menu-sm dropdown-content bg-base-100 rounded-box gap-1.5 z-1 mt-3 w-52 p-2 text-base shadow">
                            {navLinks}
                            {user ? (
                                <button onClick={handleLogOut} className="btn bg-gradient-to-r from-primary to-secondary hover:from-secondary hover:to-primary text-white rounded px-2 md:px-4 py-1 md:py-2 text-base">Logout</button>
                            ) : (
                                <>
                                    <button onClick={() => setActiveModal("login")} className="btn btn-ghost bg-gradient-to-l hover:from-secondary hover:to-primary hover:text-white border border-primary rounded px-2 md:px-4 py-1 md:py-2 text-base">Login</button>
                                    <button onClick={() => setActiveModal("register")} className="btn bg-gradient-to-r from-primary to-secondary hover:from-secondary hover:to-primary text-white rounded px-2 md:px-4 py-1 md:py-2 text-base">Register</button>
                                </>)}
                        </div>
                    </div>
                    <Link to="/" className="flex justify-center items-center text-xl md:text-2xl font-bold">
                        <img src={logo} alt="Bengal Matrimony" className="h-7 md:h-12 w-7 md:w-12" />
                ...
                <div className="navbar-end gap-2 md:gap-4">
                    ...
                    {user ? (
                        <div className="relative">
                            ...
                            {dropdownOpen && (
                                <ul className="menu menu-sm bg-base-100 rounded-box z-10 mt-3 w-52 p-2 shadow absolute right-0">
                                    ...
                                    <li><button onClick={handleLogOut} className="btn bg-gradient-to-r from-primary to-secondary hover:from-secondary hover:to-primary text-base text-white">Logout</button></li>
                                </ul>
                            )}
                        </div>
                    ) : (
                        <div className="flex items-center gap-2">
                            {/* Login Modal */}
                            <div>
                                <button onClick={() => setActiveModal("login")} className="btn btn-ghost bg-gradient-to-l hover:from-secondary hover:to-primary hover:text-white border border-primary rounded px-2 md:px-4 py-1 md:py-2 text-base hidden md:block">Login</button>
                                <Login isOpen={activeModal === 'login'} onSwitch={(modal) => setActiveModal(modal)} />
                            </div>
                            {/* Register Modal */}
                            <div>
                                <button onClick={() => setActiveModal("register")} className="btn bg-gradient-to-r from-primary to-secondary hover:from-secondary hover:to-primary text-white rounded px-2 md:px-4 py-1 md:py-2 text-base hidden md:block">Register</button>
